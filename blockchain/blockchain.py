...
class BlockChain:
    def __init__(self):
        self.adjust_difficulty_blocks = 5
        self.difficulty = 6
        self.block_time = 30
...
if __name__ == '__main__':
    block = BlockChain()
    # block 0: genius block
    block.create_genesis_block()

    # background verification and adjusting difficulty
    t = threading.Thread(target = background_job, args = (block, ))
    t.start()

    # client: mine block and add transactions
    block.mine_block('mark')
    block.pending_transactions.append(Transaction('test123', 'test456', 100, 1, 'Test'))
    block.pending_transactions.append(Transaction('test123', 'test456', 100, 1, 'Test'))
    block.mine_block('jimmy')
    
    # the fake transaction will be found when verifying
    print("Insert fake transaction.")
    fake_transaction = Transaction('test123', 'test456', 100, 1, 'Test')
